...
3. **Design System & Token Efficiency Rule**:
   - Read design skill files (`skills/impeccable-design/SKILL.md`, `skills/motion/SKILL.md`) **only when creating major new UI features or redesigns**.
   - For small edits, bug fixes, or minor tweaks, use existing CSS variables in `src/index.css` without re-reading skill files to save context tokens.
   - Design aesthetics: **NO glassmorphism**, **NO border strokes**, support **both Dark & Light modes**, classy, minimal, playful, restrained (Emil Kowalski style).
...
5. **Testing & Anti-Regression (5-Layer Quality Harness)**:
